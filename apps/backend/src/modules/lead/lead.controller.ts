...
export class LeadController {
  ...
  list = asyncHandler(async (req: Request, res: Response) => {
    ...
    const viewer = { id: req.user!.id, permissions: req.user!.permissions };
    await respondWithList(res, query, () => this.leadService.list(req.tenantContext, query, viewer));
  });

  getById = asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params as unknown as IdParam;
    const viewer = { id: req.user!.id, permissions: req.user!.permissions };
    ApiResponse.success(res, await this.leadService.getById(req.tenantContext, id, viewer));
  });
  ...
  transition = asyncHandler(async (req: Request, res: Response) => {
    ...
    const lead = await this.leadService.transition(req.tenantContext, id, statusId, req.user!.id, remarks);
